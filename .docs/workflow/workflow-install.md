...
Event Horizon installs two Copilot-facing assets into a target repository so the
workflow is both discoverable and always on.

## Source assets in this repo

- `.flux/skills/event-horizon-agent.md` is the reusable skill document.
- `.flux/skills/event-horizon-copilot-instructions.md` is the always-on
  instructions template used to patch Copilot instructions.
...
- The skill explains the workflow in detail and gives agents a discoverable
  reference surface.
- The Copilot instructions block makes the critical ticket rules always apply,
  even if the agent never reads the skill file explicitly.
