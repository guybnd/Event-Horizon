...
7. **[MANDATORY] Upload Mac and Windows build artifacts** to the GitHub release. Find the latest builds (e.g. `event-horizon-macos.zip` in the repo root and `engine/dist/event-horizon-win.exe`) and attach them with `gh release upload <version> <mac-artifact> <win-artifact> --repo guybnd/Event-Horizon --clobber`. Both platforms must be uploaded every release.
