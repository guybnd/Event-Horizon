...
7. **[MANDATORY] Build and upload Mac and Windows artifacts** to the GitHub release:
   - Build versioned artifacts by running `VERSION=<version> npm run package --workspace engine` from the repo root (or `cd engine && VERSION=<version> npm run package`). This produces `releases/event-horizon-macos-<version>.zip` and `releases/event-horizon-win-<version>.exe` at the repo root.
   - Upload both with `gh release upload <version> releases/event-horizon-macos-<version>.zip releases/event-horizon-win-<version>.exe --repo guybnd/Event-Horizon --clobber`.
   - Both platforms must be built and uploaded every release. The `releases/` directory is gitignored — artifacts live there only until uploaded.
8. Notify the user that the release was successfully created, the tickets were moved to `Released`, committed, published to GitHub with build artifacts attached, and point them to the generated release notes doc or the Releases view.
