...
3. Run `npm run flux:release <version>` in `engine/`. This gathers Done tickets, generates release notes in `.docs/`, appends a one-line-per-ticket block (with completion gist) to the canonical `<releaseNotesPath>/INDEX.md`, and moves tickets to `Released`.
