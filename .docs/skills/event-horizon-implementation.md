...
9. If clarification is required during implementation, do not ask only in chat. Leave one explicit question in ticket history and **set `"requireInput": true` in the same `PUT /api/tasks/:id` call** — the engine will atomically transition the ticket to the configured user-input status. Never post a question as a history comment without `"requireInput": true` in the same request.
