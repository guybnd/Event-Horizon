...
| POST | `/api/group/create-parent` | Create a brand-new **dedicated parent** repo to host a group (mkdir + `git init` + scaffold store + write `group.json` + register). Body: `{ parentPath, name, members: [{ name, role, remote, testCommand? }] }`. Refuses to clobber an existing `group.json` (caller routes to repair/backfill). Returns `{ parentRoot, groupName, gitInitialized, wroteConfig, scaffoldedStore, registered }`. |
...
| POST | `/api/group/promote-docs/plan` | **Parent-only** dry run: walk this repo's `.docs/` and propose a store-relative target per file (default `features/<basename>`, retargetable). No body, zero mutation. Returns `{ parentRoot, candidates: [{ source, target }] }`. A member workspace gets "open the parent" (resolves `getGroupContext()` only). |
| POST | `/api/group/promote-docs/apply` | **Parent-only** promotion with **move semantics**: write each selected `.docs/` file into the canonical store, `git rm` it from main, commit the removals, then `syncGroup` (commit on `flux-group-docs` + fan out). Body: `{ selections: [{ source, target }] }` (`source` under `.docs/`, `target` store-relative; `..`/absolute/`.git` rejected). Per-file isolation. Returns `{ promoted: string[], failed: [{ source, target, error }], sync: GroupSyncResult }`. A promoted doc is no longer on main. |
