...
| GET | `/api/config/modules/catalog` | Returns the array of built-in `ModuleDeclaration` templates (e.g. Serena) that users can add to their config. Each template has `id`, `name`, `description`, `enabled` (default `false`), optional `mcpServer`, `promptFragment`, `phases`, and `conditions`. |
