...
| Method | Path | Purpose |
|--------|------|---------|
| GET | `/api/tasks` | All tickets (array), or pass `?active=true` to get only non-terminal tickets (excludes `Done`/`Released`/the configured archive status — same definition as MCP `list_tickets`' active screen, `getTerminalStatuses()` in `task-store.ts`). **FLUX-970**: the portal's routine 3s poll uses `?active=true` — on a large board most tickets are resting Released/Archived/Done ones that never change, so shipping/parsing/diffing them every 3s was pure waste; the board already filters them out client-side anyway. Callers that need everything (Releases screen, global search) still use the unfiltered default, refreshed on mount/focus/SSE-triggered reconciles rather than every tick. Each ticket carries a **capped** `cliSessions[]`: only active sessions plus the most-recent completed run group, with each `liveOutput` truncated to a short tail (~2KB). **History is replaced by a compact `historyDigest` (FLUX-725)** so a large board doesn't parse the full array on every poll/SSE. The digest pre-computes everything the cards + attention surfaces derive from history: `{ length, lastEntry:{date,type}, lastActivityAt, enteredCurrentStatusAt, isSpeedDemon, statusChanges24h:[{from,to,date}], comments:[{id,user,date}] (no text), requireInput:{question,setDate}\|null }`. The list `history[]` is pruned to only the entry types a card renders **inline** — `comment` (the hover comment popover shows text + threaded replies) and the **active** `agent_session` (the card's inline live-progress, keyed by sessionId); every `status_change`, `activity`, and **terminal** `agent_session` (with its long `finalMessage`/`summary`/token metadata) is dropped. Use `GET /api/tasks/:id` for the **full** `history`. |
