...
| GET | `/api/sync-status` | Current sync state (`idle`, `syncing`, `synced`, `conflict`, `error`). The `error` shape is `{ state:'error', error, errorType:'network'\|'auth'\|'conflict'\|'unknown', remediation? }`; for `errorType:'auth'` a `remediation: { reason, commands[] }` payload carries the engine-owned fix steps (FLUX-895). |
