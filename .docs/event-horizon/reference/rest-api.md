...
| GET | `/api/tasks/:id/transcript` | Durable conversation transcript (raw tier) parsed into ordered chat messages (`{ role, text, ts }`) for the chat pane. **FLUX-661:** an edit-ish tool row (`Edit`/`Write`/`MultiEdit`/`NotebookEdit`) additionally carries `tool` (normalized name) + `path` (repo-relative file), so the portal can render an expandable inline diff of that file. **FLUX-674:** a user turn that carried pasted images additionally carries `attachments` (`{ url, path, fileName }[]`), rendered inline in the user bubble (survives reload). Special-cases the `__board__` sentinel. |
