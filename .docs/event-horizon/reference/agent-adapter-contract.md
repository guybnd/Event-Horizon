...
Declaring them `role: 'lead'` is deliberate, not incidental: `disallowedEhToolsForPersona` (FLUX-1434's per-role `event-horizon` MCP tool deny-list, see below) only ever scopes `role: 'worker'` personas, and `resolvePersonaPrompt` only composes `PHASE_CONTRACTS` onto non-lead personas — `lead` keeps every solo/dispatched session byte-identical to its pre-FLUX-1226 behavior (never EH-tool-scoped, never contract-composed) by construction. The resolved phase-default persona's id is never stamped onto `session.personaId` — role resolution is prompt-text-only, so the FLUX-1434 tool-scoping path (keyed on `session.personaId`, computed independently in `claude-code.ts`'s `disallowedToolsArgs`/`stampDisallowedEhTools`) is untouched by this migration.

Persona resolution precedence for a solo/dispatched session is: an explicit `personaId` (when one resolves) beats the phase's built-in default; there is **no user phase-override tier** — a custom persona's `phases` field only ever makes it *selectable for delegation* (`getSelectablePersonas`), never auto-owning a phase's solo sessions. (An explicit-`personaId` caller into `resolveSoloChatPersona` doesn't exist yet — the parameter is there for a future caller, e.g. FLUX-1229/1462, without another signature change.) `buildInitialPrompt` also exposes a single named composition seam (`resolvePhaseOverlay` in `shared.ts`, applied after role+module assembly and before session mechanics) for FLUX-1229's planned per-project additive prompt overlay — a no-op today (always returns `''`), preserving the byte-compat snapshot gate described next.

**Byte-compat gate.** `engine/src/agents/phase-persona-byte-compat.test.ts` (C1) snapshots `buildInitialPrompt`'s resolved text per phase; `engine/src/agents/claude-code-disallowed-tools.test.ts`'s FLUX-1226 (C2) block asserts `disallowedToolsArgs`/`disallowedEhToolsForPersona` stay unscoped for every phase. Both must stay green — a diff in either means the migration changed real agent-facing behavior, not just where the text lives.
