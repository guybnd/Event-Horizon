...
How the engine talks to CLI coding agents (Claude Code, Copilot CLI, Gemini CLI, Codex CLI), and what you have to implement to add a new one. Source of truth: [`engine/src/agents/types.ts`](../../../engine/src/agents/types.ts), [`engine/src/agents/index.ts`](../../../engine/src/agents/index.ts), and the four existing adapters under `engine/src/agents/`.
...
`effort` (FLUX-1629): Codex has no bare `--effort` flag — it takes a `-c key=value` config override (`-c model_reasoning_effort="<level>"`), so `CliCapabilities.effort` was widened from `{ supported: boolean; flag?: string }` to `{ supported: boolean; flag?: string; configKey?: string }`. Claude/Copilot keep `flag` (a literal a generic caller may `args.push(flag, level)` with); Codex sets `configKey` instead and `flag` stays undefined, so a generic consumer can't misuse it as `args.push(undefined, level)`/emit an invalid `-c high` — `codex.ts`'s `buildCodexEffortArgs` composes the `-c key=value` shape itself from `configKey`. Exactly one of `flag`/`configKey` must be set when `supported` is true (enforced by `adapter-contract.test.ts`'s B.1 completeness test).
...
| `effort` | Whether the CLI accepts an effort dial, and how (`{ supported: boolean; flag?: string; configKey?: string }`). `flag` is a bare CLI literal (Claude/Copilot's `--effort`); `configKey` is a `-c key=value` config-override key for CLIs with no bare-flag form (Codex's `model_reasoning_effort` — FLUX-1629). Exactly one is set when `supported` is true. Folded in from the former per-adapter `PROVIDER_CAPABILITIES` tables, which disagreed (FLUX-900, audit A.8). |
| `persistentChat` | Chat-turn clean exit → `waiting-input` (not `completed`) so the next message resumes the same session. Claude and Codex (FLUX-1630); copilot/gemini go `completed` (still resumable via `resumeSessionId`, but the first turn isn't persistent). Distinct from `resume`. (FLUX-901, audit B.1; FLUX-1630) |
...
| `spawnTimeMcpConfig` | Accepts a per-spawn MCP config file with phase/tag profile filtering. Claude (`--mcp-config`), Copilot (`--additional-mcp-config`, a different flag/JSON shape — FLUX-984), and Codex (`-c mcp_servers.<name>.url=…`, a per-key config override rather than a JSON blob — FLUX-1625); gemini `false`. Codex's `-c` injection ALSO carries the FLUX-1213 per-conversation header override — a nested `-c mcp_servers.<name>.http_headers.<key>=…` path round-trips correctly (live-verified against codex-cli 0.146.0, `codex mcp list --json`), so a Codex session's HITL prompts route to their own ticket exactly like Claude/Copilot's. (FLUX-901, B.6; FLUX-984; FLUX-1213; FLUX-1625) |
| `imageAttachments` | Resolves pasted image attachments into the resumed prompt. Claude and Codex (`-i/--image`, confirmed present on `codex exec` — FLUX-1625); Copilot/Gemini `false`. (FLUX-901, B.7; FLUX-1625) |
| `chatEditGateEnforced` | The FLUX-926 ticket-chat file-edit gate (`disallowedToolsArgs`) is a REAL block, not just an advisory prompt note. **Claude-only.** Claude enforces it via a per-tool `--disallowed-tools` deny. Codex's `-s read-only` sandbox mode COULD block all writes the same coarse way (confirmed in isolation, FLUX-1625 Phase 0) — but **FLUX-1631** found that same sandbox also silently cancels every *mutating* event-horizon MCP call regardless of mode, and the only fix (`--dangerously-bypass-approvals-and-sandbox`) lifts the sandbox too; `codex.ts`/`codex-board.ts` now spawn with that bypass unconditionally, so the sandbox is never actually in force and this reads `false` — enforceable in principle, not enforced in the shipped configuration. Copilot/Gemini expose neither mechanism at all. All three non-Claude frameworks instead get `chatEditGateNote`'s best-effort instruction (`shared.ts`), wired into `buildInitialPrompt`'s `editsGated` option and `prependEditGateNote` on resume. The FLUX-1443 scratch-ticket gate (`isScratchSession`) is layered onto this same mechanism and gets the same enforced-vs-advisory split — see `branch`/`finish_ticket` in [mcp-tools.md](mcp-tools.md) for the framework-agnostic hard blocks that close the rest of that gap. (FLUX-1123 / FLUX-1443 / FLUX-1625 / FLUX-1631) |
...
Called when the user types into an active session. All four adapters forward to a shared `sendCliSessionInput(...)` that:
...
> **Start with `CAPABILITY_PROBES`** (FLUX-1626) — the typed checklist immediately above `CLI_CAPABILITIES` in [`types.ts`](../../../engine/src/agents/types.ts). It carries one entry per capability flag plus the cross-cutting preflight concerns that are *not* flags (loopback MCP reachability, resume-id semantics, stdin prompt delivery, worktree commits, `terminalReason` error strings, spawn-vs-resume flag divergence, …), each with the exact live-CLI probe that settles it and what counts as a pass. It lives in code rather than here on purpose: this page is not open while you are staring at a red `Record<CliFramework, …>`, and every entry in that table exists because a shipping adapter once got it wrong (FLUX-959, FLUX-977, FLUX-984). **Settle each row against the live CLI — never from its documentation.**
...
## Per-connector read/write scoping (FLUX-1657)

Distinct mechanism from the per-phase server profiles above (those hide an entire server; this hides only a server's *mutating* tools). Config field `mcpServerReadOnly: Record<serverId, boolean | { exceptPhases?: LaunchPhase[], allow?: string[], deny?: string[] }>` (not seeded in `CONFIG_DEFAULTS` — absent means no scoping, persisted via `patchConfig` through the targeted `GET`/`PUT /api/config/mcp-readonly` routes, never a full-config write — see [REST API Reference](rest-api.md)).

Enforcement lives in [`mcp-readonly.ts`](../../../engine/src/mcp-readonly.ts) and reuses the **same deny-list layer** `disallowedToolsArgs` (`claude-code.ts`) already uses for `FILE_MUTATION_TOOLS`/EH tool scoping — appending exact `mcp__<serverId>__<tool>` names to `--disallowed-tools`, which (per FLUX-1376's empirical finding) drops the tool's schema from what the model even sees, not just its call permission:

- **Tool-name resolution is probed, not hand-listed.** `getConnectors()`'s real tool names are resolved via `probeMcpServer` (`mcp-schema-probe.ts`, the same `client.listTools()` probe the `GET /api/tasks/debug/mcp-schemas` report uses) and cached in-process by server id (`refreshMcpServerTools`/`getCachedToolNames`). The cache is warmed whenever a connector is probed for connectivity (`probeConnector`, `module-probe.ts`) — both on the Connectors panel's manual Test button/SSE-driven checks (FLUX-1656) AND automatically on every workspace activation (`probeAllConnectors(getConnectors())`, fired alongside `probeAllEnabled` in `task-store.ts`'s `hydrateWorkspace`, FLUX-1659), so a fresh engine start warms the cache without waiting on a human click. Either way it is **never synchronous at session-spawn time**, so a slow/unreachable connector can't add spawn latency.
- **Mutating-tool detection is pattern-based, not hand-listed** (unlike `FILE_MUTATION_TOOLS`, which enumerates fixed EH-adjacent tool names): a tool name matching `create|update|delete|write|add|remove|transition|publish` (case-insensitive) is treated as mutating, minus a per-server `allow` override, plus a per-server `deny` override for an oddly-named write tool the pattern misses. Pure decision function: `mutatingToolsForServer(serverId, toolNames, cfg, effectivePhase)`.
- **Phase source is `LaunchPhase`** (`effectivePhase = session.handoffPhase ?? session.phase`, the same value `disallowedEhToolsForPersona` reads) — **not** `SCOPE_PHASES` (`routes/config.ts`, used by `/mcp-phases` above), which omits `finalize`. A rule's `exceptPhases: ['finalize']` reveals a connector's write tools only for a finalize-phase session; every other phase keeps them hidden.
- **Fails open by construction, twice over:** a server absent from `mcpServerReadOnly` is never scoped, and a server that IS configured read-only but has no cached tool names yet (never probed, or the last probe failed) also resolves to `[]` — an unresolved connector is left **fully enabled**, never blind-stripped. Mirrors the FLUX-242 "missing tool" lesson the EH-tool deny-list model (`orchestration-personas.ts`) already encodes.
...
- `checkBinaryInstalled` (Claude/Gemini/Codex) or `checkCopilotBinaryInstalled` (Copilot, FLUX-1641) throws a user-facing error if the CLI is missing. Routes surface it as a 400 with the message; a dispatched (non-chat) session instead gets it inline via `appendErrorToSession` (`routes/cli-session.ts`'s `spawnSession`/`prepareAndLaunchSession`).
  - `checkBinaryInstalled` ([`shared.ts`](../../../engine/src/agents/shared.ts)) is PATH-only (`which`/`where`) **except for Claude on darwin** (FLUX-1600), where a login-shell alias/function can resolve `claude` to a different binary than a bare `which` sees — that path validates the SAME resolution the adapter would actually spawn instead. A positive result caches forever; a negative result negative-caches for 30s (`BINARY_CHECK_NEGATIVE_TTL_MS`), but only when `isDefinitiveNotInstalled` confirms the checker's own process exited cleanly non-zero — a transient failure (the checker killed by its own 10s timeout under load) is never cached, so it can't mask a real install for the TTL window (FLUX-985/996).
  - `checkCopilotBinaryInstalled` (`copilot.ts`, FLUX-1641) exists because Copilot's resolver also accepts a VS Code globalStorage `copilot` binary and (Windows only) a node+npm-loader.js fallback, not just PATH — a PATH-only check would falsely reject those installs. It validates the SAME resolution `spawnCopilot` would actually use (mirroring FLUX-1600's resolved-path pattern) and, like `checkBinaryInstalled`, never negative-caches a "not found" result — `resolveCopilotBinaryUncached` swallows every probe failure with a bare `catch {}`, so it cannot distinguish a definitive miss from a transient timeout, and caching either as "not installed" risks masking a real install. Until FLUX-1641, Copilot skipped this step entirely (`checkBinaryInstalled` was never called), so a missing CLI surfaced as a bare `spawn copilot ENOENT` instead.
