...
### Per-role toolset scoping — deny-list model (FLUX-1434, replaces FLUX-1385)

Every session above connects to the **same** MCP server and, by default, sees every tool's full schema — expensive for a worker delegate (a Context Scout, Requirements Interrogator, or single-lens reviewer) that only ever calls a handful of them. Claude Code sessions (only — see the capability-flag caveat below) scope this down by **persona role**, computed fresh at every spawn *and* resume:

```
deny = categoryDeny[persona.role]      // the cost lever, config-overridable
     − persona.enableTools             // per-persona re-enables
     − phaseBaseline[launchPhase]      // standalone/lead positions only
     − dispatch.enableTools            // explicit per-launch grant
     − NEVER_DENY                      // undeniable floor
```

- `disallowedEhToolsForPersona(ctx: ToolScopingContext)` (`engine/src/orchestration-personas.ts`) — `ctx` is `{ personaId?, phase?, patternPosition?, enableTools?, focusComment? }`. Returns the bare tool names to disallow for a `role: 'worker'` persona; `undefined` (no restriction — the full toolset) for `role: 'lead'`/`role: 'flex'` personas and any unresolvable `personaId`, so an ambiguous or ad-hoc (non-persona) launch always fails **open** toward more tools, never fewer.
- **`CATEGORY_DENY_DEFAULTS[role]`** is the shipped deny list per role (`worker` ≈ 26 tools; `lead`/`flex` deny nothing). It is hand-maintained, **not** derived from "every registered tool" — a newly registered MCP tool absent from it is simply never scoped down for anyone (fails open to available, the opposite failure mode from FLUX-1385's allow-list, which failed a new tool closed until an override was added). Board-config overridable via `toolScoping.categoryDeny.<role>` (`engine/src/config.ts`) — a full replacement array for that role, not a merge.
- **`NEVER_DENY`** (`get_ticket`, `add_note`, `ask_user_question`, `permission_prompt`) is an undeniable floor, applied last regardless of role/config/persona — `add_note`+`get_ticket` let a stranded delegate always leave a trace; `permission_prompt` fixes FLUX-1385's regression #6 (a worker in gated permission mode losing the very tool `--permission-prompt-tool` routes every decision through) by construction.
- **`persona.enableTools`** (declared on `OrchestrationPersona`, built-in or custom via `saveCustomPersona`) re-enables specific tools for that persona regardless of launch position — e.g. `implementer: ['change_status']`, `regrounder: ['update_ticket', 'change_status']`. A custom `role: 'worker'` persona with no declaration gets the role default (`CATEGORY_DENY_DEFAULTS.worker` alone) — first-class, not silently under-scoped (FLUX-1385 regression #4).
- **`PHASE_BASELINE[phase]`** re-enables what that launch phase's own generic mission text instructs (e.g. `grooming`/`fast-path`/`batch-grooming` → `update_ticket, change_status, create_ticket, publish_artifact`; `review` → `change_status, create_ticket, update_ticket`) — but **only** for `standalone`/`lead`/`combiner` pattern positions. A delegate (`assistant`/`step`) never gets it: its contract defers status moves to its lead, and the token savings live specifically in delegates. This is what fixes "any standalone launch of a phase with a worker persona" as a class (portal Start Task persona picker, `start_session` with `personaId`) — previously only a persona's own override table could re-enable a tool.
- **`dispatch.enableTools`** is an explicit per-launch grant (the `/start` and `/delegate` route bodies' `enableTools` array, the `delegate` MCP tool's per-delegation `enableTools`) — applies regardless of pattern position, same as `persona.enableTools`. The Furnace's review dispatch (`furnace-stoker.ts`'s `reviewDispatchOpts`) sends `enableTools: ['furnace_ticket']` so a sole reviewer can use the batch follow-up mechanism `furnaceFollowupFocus` authorizes (FLUX-1385 regression #2).
- **Resume seam:** `disallowedToolsArgs` recomputes the deny list from the session record on *every* spawn and resumed turn, so a session's toolset always matches its current mission. The Furnace/gate-runner "resume, don't respawn" seam (`furnace-stoker.ts`'s `findResumeCandidate`) only ever resumes into a `standalone`/`lead`-position session — never a completed delegate (`assistant`/`step`), whose scoped-down toolset belonged to whatever lead spawned it (FLUX-1385 regression #1: a plan-revise resume landing in a `requirements-interrogator` delegate that no longer held `update_ticket`). `resumeOrDispatchSession` also re-stamps `dispatch.enableTools` onto the resumed session record before the resume POST.
- `claude-code.ts`'s `disallowedToolsArgs` consumes this list, prefixes each name with `mcp__event-horizon__`, and appends it to the same `--disallowed-tools` CLI flag that already blocks `AskUserQuestion`/`ScheduleWakeup`/the chat file-edit gate below — and is skipped entirely for `phase === 'chat'` sessions (interactive; savings negligible, confusion high). **FLUX-1376 empirically confirmed** this flag drops the listed tools' *schemas* from what the CLI sends the model, not just their call permission (`cache_creation_input_tokens` fell ~15k over 28 disallowed tools in a back-to-back spawn comparison) — so this needed no separate conditional tool-registration mechanism in `buildMcpServer()` itself, and the safety boundary (a disallowed tool call is genuinely refused, not merely hidden) holds independent of whether a given persona's scoping actually saves tokens.
- **Observability:** the computed deny list is stamped on the session record (`disallowedEhTools`, `CliSessionRecord`/`CliSessionSummary`) at every spawn/resume (`stampDisallowedEhTools` in `claude-code.ts`) and shown read-only in the portal's ticket chat `CliSessionPanel` when non-empty — a session missing a tool its mission needs is a one-glance diagnosis instead of a re-derivation.
- `personaId`/`focusComment`/`patternPosition`/`enableTools` are threaded onto the session record (`CliSessionRecord`, internal-only — not part of the client-facing `CliSessionSummary`, except the read-only `disallowedEhTools` output above) from every launch site that resolves a persona (`routes/cli-session.ts`'s `start`, `delegate`, and the deferred relay-step launcher).
...
| Annotation | Tools |
|------------|-------|
| `readOnlyHint: true` (+ `openWorldHint: false`) | `get_ticket`, `get_session_log`, `read_skill`, `list_tickets`, `get_board_config`, `get_project_group`, `list_available_agents`, `get_board_state`, `list_workspaces`, `permission_prompt` |
...
| [`list_workspaces`](#list_workspaces) | Read | — |
...
- **`bodyVersion`** (FLUX-1550): an opaque content-hash CAS token for the current `body`, computed over the full (untruncated) body regardless of the truncation above. Echo it back as `baseBodyVersion` on [`update_ticket`](#update_ticket) to detect a concurrent body edit before overwriting it — see that section for the full contract.
...
**Output:** returned as `structuredContent` (the text `content` block is empty — see [Structured output](#structured-output-flux-950)): `{ statuses, projects, tags, priorities, users, requireInputStatus, readyForMergeStatus, workspaceRoot, binding, storeMode }`. `statuses` merges visible columns and hidden statuses.
...
**Workspace-binding disclosure (FLUX-1573).** A hand-launched session using a repo's static `.mcp.json` sends no `X-EH-Workspace` header, so it silently binds to the boot/default board (`getWorkspace()`'s deterministic fallback, FLUX-1557) with no way to tell it apart from a verified, header-bound session — the exact gap that let a misbound orchestrator confabulate its binding and cross-board-ignite another board's Furnace batch. Three fields close that gap:

- `workspaceRoot` — the **canonical** (realpath'd, `canonicalizeWorkspaceRoot()`) root this session is bound to, or `null` if unset.
- `binding` — `'header'` when this connection carried a resolvable `X-EH-Workspace` header (verified), or `'default-fallback'` when it carried none and silently resolved to the boot board (unverified — treat as a guess before any mutation).
- `storeMode` — `'in-repo'` (`.flux/`) or `'orphan'` (`.flux-store/`), from `isOrphanMode()`. Previously agents guessed this from the rules doc's either/or phrasing and stated the guess as fact.

**Version probe:** if `workspaceRoot`/`binding`/`storeMode` are **absent** from the response entirely, the connected engine predates FLUX-1573 — treat the workspace binding as unverified and confirm it out-of-band before any board mutation (`list_workspaces` will also be missing on such an engine).

### `list_workspaces`

Read-only: list every registered workspace, mirroring the REST `WorkspaceInfo` shape (`GET /api/workspaces`, [`engine/src/routes/workspaces.ts`](../../../engine/src/routes/workspaces.ts)) — the same enrichment (`enrichList`/`enrichEntry`) is reused so the two never diverge.

**Input:** none.

**Output:** `structuredContent: { workspaces: WorkspaceInfo[] }`, one entry per registered board: `{ path, canonicalRoot, label?, displayName, active, available, open, closable, liveSessionCount, group? }`.

- `canonicalRoot` (FLUX-1573) is the field to copy into an `X-EH-Workspace` header — it is the realpath'd registry key (`canonicalizeWorkspaceRoot(path)`), unlike `path`, which is only `path.resolve()`d. A short-name or differently-cased-but-same-file `path` would miss the registry lookup and silently fall back to the default board (the FLUX-1571 failure mode); `canonicalRoot` routes byte-exactly.
- **No self-service rebinding.** This tool never changes which board the *current* session is bound to — it is pure discovery. See [`get_board_config`](#get_board_config)'s `workspaceRoot`/`binding` fields for the current session's own binding, and the workspace-binding disclosure note above for the version-probe convention (a pre-FLUX-1573 engine has no `list_workspaces` tool at all).
- Registered `readOnlyHint: true` and in the `permission_prompt` auto-allow tier (`SAFE_PERMISSION_TOOLS`).
...
| `baseBodyVersion` | string | **FLUX-1550** — optimistic-concurrency token for a `body` replacement, echoed back from a prior `get_ticket`'s `bodyVersion`. See below. |
...
**Body CAS (`baseBodyVersion`, FLUX-1550).** `get_ticket` serves a `bodyVersion` — an opaque content-hash of the current `body` — alongside every ticket. A `body`-replacing `update_ticket` call that also passes `baseBodyVersion` is checked against the ticket's *current* on-disk body version (re-read fresh, under the per-ticket write lock) before the write applies: a match writes normally; a mismatch means someone else changed the body since this caller last read it, and the call is rejected with `isError: true`, `structuredContent.code: 'invalid_state'`, and `structuredContent.currentBodyVersion` set to the fresh version — re-read via `get_ticket` and reapply the edit rather than retrying blind. **Omitting `baseBodyVersion` is grandfathered**: the write still applies unconditionally (today's pre-CAS behavior, i.e. last-writer-wins) and the engine logs a warning — safe for a metadata-only call (no `body`), but a `body`-replacing call that skips the token can silently clobber a concurrent edit. Metadata-only calls (no `body`) are never gated regardless of `baseBodyVersion`. The REST twin (`PUT /api/tasks/:id`) takes the same `baseBodyVersion` field and responds `409 { error: 'stale_body', currentBodyVersion, message }` on a mismatch.
...
- **Unowned changes-requested bounce → visible Needs-Action (FLUX-1681).** A `changes-requested` verdict on this same call that moves the ticket **off** `Ready` is checked against `isChangesRequestedBounceOwned` (`temper.ts`): is Temper already looping this ticket, or is it owned by an active Furnace batch? If **neither** owns it (e.g. the ticket's `review` gate isn't `auto`, or it's branchless — a manual/human-led review path), `shouldFlagUnownedChangesRequested` (pure, `mcp-server.ts`) flags it — this call writes [`needsAction`](ticket-schema.md) with a durable reason and raises the `require-input` swimlane in the SAME write as the status move, plus a `generateNeedsActionNotification`. This deliberately does **not** dispatch any remediation session — auto-continuing a manual/human-led review is out of scope — it only makes the stall visible instead of leaving the ticket silently unattended in its working column (the reported ANZUBRAI-51 incident). Never fires for a bounce Temper/Furnace already owns, so it can't race or double-flag their own in-flight continuation.
...
Each call is a **new revision** (history is kept — never an overwrite). The HTML is stored in a traversal-guarded sidecar at `.flux/artifacts/{ID}/{rev}.html` (never inlined in the body). The tool broadcasts `taskUpdated` + `artifactReady { ticketId, rev, channel }` over SSE, and the portal renders the artifact in a sandboxed iframe via [`GET /api/tasks/:id/artifact`](rest-api.md).

**FLUX-1667 — two typed artifact channels, each with its own frontmatter pointer + revision history.** `channel: 'plan'` (the default) writes the `artifacts: { latest, revisions[] }` pointer — grooming mockups and Ready-time Visual Recaps, surfaced in the ticket's **Plan** tab. `channel: 'doc-recap'` writes a sibling `docRecap: { latest, revisions[] }` pointer — surfaced in the **Docs** tab — sharing ONE ascending rev-number counter with `artifacts` per ticket (revision files for both channels sit in the same `.flux/artifacts/{ID}/` directory; a concrete `?rev=N` GET resolves off disk regardless of channel, but resolving `latest` needs `?channel=doc-recap`). An agent normally never passes `channel:'doc-recap'` explicitly — see the auto doc-recap paragraph below — it exists for hand-authoring a recap outside that automatic flow.

**FLUX-1662 — auto doc-recap, a separate (non-tool) emitter sharing this same persistence and channel.** When a ticket's branch touches a docsRoot `.md` file, the engine itself — not this tool, and with zero agent tool calls — publishes a `kind:'doc-recap'` revision into the `docRecap` channel via the same `withArtifactPublication`/`writeArtifactRevisionInPublication` primitives this tool uses: on the ticket's `change_status → Ready` transition (both the MCP path and the portal's REST drag-to-Ready), and again after a self-serve inline doc edit lands via `POST /api/diffs/file/commit` (see rest-api.md). Each doc-recap revision also carries `docPaths` — the ordered set of doc paths actually rendered inline (≤5) — which the portal's per-doc tab strip reads from the currently-selected revision to build its tabs. A **legacy ticket** written before this split may still have a `kind:'doc-recap'` revision mixed into `artifacts`; the engine's read-path migration (`partitionArtifactChannels`, task-store.ts) splits it into the two channels in-memory on every load (never persisted back to frontmatter), so every REST/portal read sees the split shape regardless of on-disk shape.
...
| `channel` | `'plan' \| 'doc-recap'` | no — default `'plan'`. See the FLUX-1667 paragraph above. |
...
- **CI gate (FLUX-560):** for a branch ticket, refuses to merge on a failing or (by default) pending GitHub check-rollup verdict — stack-agnostic by construction: EH only reads `gh`'s `statusCheckRollup` (`checks.failed`/`checks.pending`, already surfaced via `PrStatus.checks` on `GET /:id/pr` and the portal's PR deck card), never runs the user's stack itself. `checks.total === 0` (no GitHub CI on the repo) never gates by itself; if the board config's `ci.checkCommand` is set, that command runs instead and its exit code decides (unset = no gate for a no-CI repo). Refusal leaves ticket status **unchanged** (mirrors the shared-PR guard above) and names the failing/pending count; re-run with `force: true` (the same param as the shared-PR guard) to merge anyway. The portal's own merge route (`POST /:id/pr/merge`) applies the identical gate before merging, returning a 409 with `ciBlocked: true` + `requiresForce: true` (same shape as its `sharedNonDone` 409). `ci.gate: 'off'` restores pre-FLUX-560 unconditional-merge behavior; `'warn'` never refuses but the reason still comes back for logging. See [Configuration Reference](../configuration.md#ci-gate).
...
- **Scratch guard (FLUX-1443):** refuses outright for a `kind:'scratch'` ticket — a scratch has no PR/implementation lifecycle to finish. Points the caller at [`extract_ticket`](#extract_ticket) (or a board-rebase `promote`) to seed a real card first, then finish that promoted ticket.
...
| `delegations` | array (≥1) | yes | Each: `{ personaId, task, effort?, model?, enableTools? }`. Length 1 = serial; >1 = parallel. |
| `timeout` | number | no | Seconds for ALL delegations (default 300, max 600). |

**Output:** `[{ persona, succeeded, status, output }, …]` — one entry per delegation, in input order. A rejected delegation yields `{ persona, succeeded: false, status: 'error', output: <reason> }`. (`model` is an optional per-delegation override — **FLUX-482/931** — honored with precedence per-call `model` > `persona.modelTier` (tier-resolved per-framework) > config `delegateModel` default > the status-derived default, on all three frameworks; see [`cli-session.ts`](../../../engine/src/routes/cli-session.ts). `enableTools` — **FLUX-1434** — an optional array of bare `event-horizon` MCP tool names to grant this delegate beyond its persona's normal deny-list scoping; see [per-role toolset scoping](#per-role-toolset-scoping--deny-list-model-flux-1434-replaces-flux-1385) above.)
