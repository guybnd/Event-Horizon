...
**Multi-workspace fan-out (FLUX-1230 S7/FLUX-1513, hardened FLUX-1554).** `driveStokeTick` runs `driveBurningBatches(ws)` once per `liveWorkspaces()` entry. Every `FurnaceBatch` carries an optional `workspaceRoot` (stamped at `createFurnaceBatch` time from the caller's bound workspace — `furnace_build`'s MCP handler and the raw `POST /api/furnace` route both set it), and each pass filters the engine-global batch cache down to the batches actually owned by `ws` via `getBurningBatchesForWorkspace(ws)` / `getFurnaceBatchesCacheForWorkspace(ws)` (`batchBelongsToWorkspaceRoot` in `models/furnace.ts`). A legacy batch created before this field existed has no `workspaceRoot` to match, so it falls back to the process's default workspace (`getDefaultWorkspace()`) — with exactly one live workspace (today's only reachable configuration) every batch, tagged or not, still resolves there. `refreshWorktreePool` / `checkFurnaceSlotHealth` stay engine-global (the worktree pool and slot cap are one shared budget across all boards) — only the per-batch tick/reconcile loop and the pool scan are workspace-scoped (the pool scan itself runs once per live workspace, keyed by root).

**Every other Furnace entry point binds to the BATCH's own board, not the ambient/active one (FLUX-1554).** FLUX-1548/1551 bound the stoke drive loop itself; everything reachable OUTSIDE that loop — `igniteBatch`, `resumeBatch`, `stopBatch`, and the recovery actions (`retryTicket`/`dismissTicketFlag`/`takeoverTicket`/`handBackTicket`) — now resolve `batch.workspaceRoot` (`resolveWorkspaceByRoot(...) ?? null` falling through to the deterministic default, the same idiom `hitl-prompts.ts`/`agents/board-core.ts` use) and wrap their work in `runWithWorkspace(ws, …)` **before** doing anything, so a call made from — or triggered against — a different board than the batch's own still lands on the batch's board:
- `checkTriggers` iterates `liveWorkspaces()` and evaluates/ignites each board's own draft batches inside that board's binding; `isTriggerSatisfied`'s `'pr'`-ref scan is scoped to the batch's own board (`getFurnaceBatchesCacheForWorkspace`) so a same-url PR that only exists on a DIFFERENT board can't falsely satisfy it.
- `reconcileBatchCached`/`reconcileAllBatchesCached` (the GET-triggered background refresh behind `GET /api/furnace`, `GET /:id`, `furnace_get`) bind each batch to its own board before reconciling — unbound, a poll from board A used to reconcile board B's batches against board A's `ws.tasks`.
- `gh` (`branch-manager.ts`'s `gh()`, used by `postPrReview`/`mergePullRequest`) runs with `cwd = requireWorkspaceRoot()` — the merge route (`POST /api/furnace/:id/merge`) binds to the batch's own root before calling `mergePullRequest`, so a merge always runs in the repo that actually holds the PR.

**The furnace-store cache/persistence is per-root, not split-brain (FLUX-1554).** Before this fix the in-memory cache's `loaded` flag was a single process-global boolean and `persist()` always wrote to `getActiveFluxDir()` (the AMBIENT board) — so a second board's `furnace-batches/` directory was never read into the cache at all, and a batch mutated while a different board was "active" wrote its sidecar into the WRONG board's directory. Batch ids are process-wide-unique UUIDs, so the in-memory `cache` itself stays one flat `Record<id, FurnaceBatch>` shared across every board (no root-scoping needed for lookups) — what changed is that `loaded` became a `Set<root>` (`ensureFurnaceLoaded(root?)` loads and merges ONE root's directory into the shared cache, marking that root loaded, rather than replacing the whole cache) and `persist()`/`deleteFurnaceBatch` always resolve the batch's OWN root (`batch.workspaceRoot ?? getDefaultWorkspace().root`) via `getFurnaceDir(root)`, never the ambient one. No on-disk layout change — still one flat `<flux>/furnace-batches/<id>.json` per board.

**MCP/REST enforce per-connection ownership (FLUX-1554, extended to REST by-id routes FLUX-1567).** A connection/request bound to board A can no longer read or mutate board B's batch just by knowing its bare id. On MCP: `furnace_get` (single + list), `furnace_update`, `furnace_batch` (ignite/stop/resume/discard), and `furnace_ticket` all gate through `batchOwnedByBoundWorkspace` (mcp-server.ts) and answer an unowned id identically to "not found" (never leaking that the batch exists on another board). On REST (`routes/furnace.ts`): `GET /api/furnace` (list) is filtered to the request's bound board via `getFurnaceBatchesCacheForWorkspace` (FLUX-1554); every by-id route — `GET/PUT/DELETE /:id`, `POST /:id/ignite|stop|resume|merge`, `POST /:id/ticket`, `DELETE /:id/ticket/:ticketId`, and `POST /:id/tickets/:ticketId/retry|dismiss|takeover|handback` — gates through the route-local `batchOwnedByBoundWorkspace` helper (mirroring the MCP one) and answers 404 identically to not-found (FLUX-1567; before this, only the list route was board-scoped and a by-id REST call could read/mutate another board's batch — the mutation helpers already self-bound to the batch's own root per FLUX-1554 above, so this was a read/authorization gap, not a data-corruption one). `validateBatchTickets`'s "already active in another batch" invariant is likewise scoped to the caller's own board (`resolveTickets` in `routes/furnace.ts`, `furnace_ticket action:"add"`/`furnace_build` in `mcp-server.ts`) — a same-id ticket queued in a batch on a DIFFERENT board no longer falsely blocks (or is conflated with) this board's check.
...
**Per-workspace-root census, ONE shared cap (FLUX-1551).** Two boards can share a bare ticket id (both use
the `FLUX-` prefix), so the observed worktree pool and Temper's own pending-slot reservations are keyed
**per workspace root** — `setObservedWorktrees(root, ticketIds)` / `ticketHasObservedWorktree(root, ticketId)`
/ `setTemperReserved(root, ticketId, reserved)` (`furnace-store.ts`) — so one board's stale census, or a
same-id ticket on a different board, can never satisfy (or clear) another board's slot check. `driveStokeTick`
calls `refreshWorktreePool({ root: ws.root })` once per **live** workspace inside its per-workspace loop
(FLUX-1551 fixed a prior single call outside that loop, which only ever scanned whichever board
`requireWorkspaceRoot()` happened to resolve to). `globalSlotsInUse` still **sums every root's usage into the
ONE `FURNACE_SLOT_CAP`** — this is a keying/census correctness fix, not a change to the budget itself. A
worktree slot is a per-**machine** resource (one heavy agent session on the user's one machine), not a
per-board one — `driveStokeTick`'s own comment states this explicitly: worktree-pool accounting is "a single
budget shared across all boards ... a design point this subtask inherits, not one it changes" (FLUX-1452).
FLUX-1551 preserves that invariant rather than reversing it: opening a second board narrows *whose* census/
reservations are read, but never multiplies the machine's concurrent-heavy-session ceiling.
