...
| `needsAction` | string \| null | **FLUX-651 / FLUX-826.** Set by the engine when an agent leaves an open item with no board action. Four triggers: (1) **hard** — a turn ends in a working status (`Grooming` / `In Progress`) with no board action; (2) **soft (FLUX-826)** — a turn ends in a resting/terminal status (`Todo`/`Ready`/`Done`/`Backlog`/…) having posted a fresh agent comment but taken no board action and raised no `ask_user_question`; (3) **question timeout (FLUX-826)** — a ticket-bound `ask_user_question` times out unanswered; (4) **unowned changes-requested bounce (FLUX-1681)** — a `changes-requested` verdict moves a ticket off Ready and no engine-owned continuation driver (Temper mid-loop, or an active Furnace batch) picks it up — written synchronously inside the same `change_status` call that records the verdict (`shouldFlagUnownedChangesRequested` + `isChangesRequestedBounceOwned` in `mcp-server.ts`/`temper.ts`), alongside the `require-input` swimlane, so a Codex/Claude ticket outside Temper's `auto` review gate is never left silently stalled in `In Progress`. Truthy = the board renders the ticket in a **"Needs Action"** group and a notification is raised; the string is the reason. Cleared automatically when the ticket's status moves, a Require-Input swimlane is raised, or work resumes on the next turn. Not hand-authored. |
