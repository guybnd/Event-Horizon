...
| iframe → host | `annotations` | `{ items:[{ id, kind?, selector, text, containerText?, label?, note? }] }` | **The current annotation set, mirrored LIVE (FLUX-1362 — on every add/edit/remove, not just on a Send).** Each item carries its stable pin `id` (so a host edit/remove round-trips back to the pin), `selector` (a CSS path to the element), `kind` (FLUX-892: `'text'` selection — the default when absent — or `'element'` right-click pick), `text` (the selected text, ≤300 chars, empty for element picks), `label` (a tag+snippet descriptor for element picks, e.g. `button "Save"`), and the user's optional `note`. The host owns the unified, editable list (the floating "N changes" pill) and composes the **single** `🎯 Artifact annotations` chat message on send — text items render `> excerpt`, element items render `⊙ \`label\``. |
