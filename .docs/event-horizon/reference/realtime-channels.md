...
|---|---|---|---|
| iframe → host | `ready` | `{ hasGuidedControls? }` | The injected script is live on the current document. `hasGuidedControls` (FLUX-1440) is `true` when the artifact declares any `data-eh-feel`/`data-eh-decision` guided-annotation control, so the host can show the staged tray's illustrated empty-state; absent/`false` for plain artifacts, which keep the existing hide-when-empty tray. |
