...
| `artifactReady` | MCP `publish_artifact` (FLUX-873) | yes — the **Grooming Artifact** panel (`ArtifactPanel.tsx`) refreshes to the new revision when the payload's `ticketId` matches the open ticket. Payload: `{ ticketId, rev }`. (Also broadcasts `taskUpdated` so the ticket's `artifacts` pointer re-loads.) FLUX-874: a fresh publish (the agent's response to an annotation) jumps the viewer to the new revision. |
