...
| `notification` | `notifications.ts`, notification routes | yes — updates notification panel + unread count. Payload `{ notification, unreadCount }`; `notification.type ∈ {error, prompt, completion, review, info}`. **FLUX-1555** added `notification.workspaceRoot` (the board that generated it) and scoped the whole notification store (reads, dedup, mark-read/dismiss) to it, so `unreadCount` here is that board's count, not the process-global one. **FLUX-922** added `review` — emitted by `generateReviewNotification(ticketId, title, verdict)`, fired from the `change_status` MCP tool when a review concludes with a recorded `reviewState` (`approved` → emerald verdict chip, `changes-requested` → amber). Portal maps `review` to the **Updates** category (`notificationCategory.ts`); the verdict chip + branch chip are portal-derived from the linked task (`reviewState` / `branch`), so the payload shape is unchanged. **FLUX-1363** added `generatePlanAutoApprovedNotification(ticketId, title)` — an `info`-typed, title-scoped-dedup entry fired from `PLAN_GATE_SPEC.onApproved` (`gate-runner.ts`) only under the `loop-auto` (`Auto`) plan-gate mode, whose approval otherwise moves Grooming → Todo with zero user-facing signal (`one-pass`/`loop-confirm` already raise a `prompt` notification asking for confirmation, so they're excluded). |
...
| `artifactReady` | MCP `publish_artifact` (FLUX-873), or the engine's own doc-recap emitter (FLUX-1662) | yes — the **Artifact**/**Docs** panel (`ArtifactPanel.tsx`; labeled **Visual Recap** for a FLUX-976 recap revision) refreshes to the new revision when the payload's `ticketId` matches the open ticket AND the payload's `channel` matches the panel's own bound channel. Payload: `{ ticketId, rev, channel }`, `channel ∈ 'plan' \| 'doc-recap'` (FLUX-1667 — the two channels share one ascending rev-number counter per ticket, so a panel MUST filter on `channel` too or an unrelated channel's publish would swing its `rev` state to a revision number its own pointer doesn't contain). (Also broadcasts `taskUpdated` so the ticket's `artifacts`/`docRecap` pointer re-loads.) FLUX-874: a fresh publish (the agent's response to an annotation) jumps the viewer to the new revision. |
...
| `authRecovered` | `agents/auth-recovery-watch.ts`'s `watchForCredentialRefresh` (FLUX-1601) — started alongside `surfaceAuthDiagnosis` (`claude-code.ts`) the instant a session is classified `terminalReason: 'auth-expired'`; polls `~/.claude/.credentials.json`'s mtime (bounded to 15 min) and fires once it changes (including the file appearing for the first time) | yes — `useChatSession`'s `authRecovered` subscriber matches on `taskId === conversationId`, sets `recovering: true`, and auto-calls `retryLast()` (one automatic retry of the failed turn) — no app restart, no re-typing the message. Payload: `{ taskId }`. |
| `bootProgress` | `initDir()` (`task-store.ts`, FLUX-1540; parallelized + boot-index-aware in FLUX-1547) — emitted at each 10-file threshold (`BOOT_PROGRESS_EVERY`) during the full-load pool, once more if a persisted boot index (`boot-index.ts`) served a partial cache hit, and a final time when the scan completes | yes — drives the cold-boot loading state (`Board.tsx`'s `BoardLoadingState`, store field `bootProgress`): a "Loaded X / Y tickets" count (tweened smoothly toward each new value via `useRollingBootCount`, FLUX-1547) + determinate progress bar in place of a static skeleton for the whole boot scan on a large board. Payload: `{ loaded, total, phase }`, `phase ∈ cached \| scanning \| ready` (`cached` only fires when a persisted boot index served some, but not all, tickets from cache without a full reparse — a fully warm boot with everything cached emits a single terminal `ready` and skips `cached` entirely). `total` is a zero-I/O in-memory filter of the already-`readdir`'d file list, computed once before the scan. The full-load pool (FLUX-1547) runs a bounded-concurrency worker pool (`concurrency.ts`) rather than a strict serial loop, so files complete out of order — the `loaded` counter and its threshold crossings stay deterministic regardless (a shared counter incremented once per completion). The portal clamps `loaded` monotonic (a late/duplicate event can't move the count backward) and falls back to an indeterminate animation if no event has arrived yet (scan finished before the SSE connection opened, or the event was otherwise missed) — never a stuck `0 / 0`. Broadcasting is wrapped so a throw can never abort the scan. |
...
The grooming-artifact iframe is **cross-origin by design** — `sandbox="allow-scripts"` *without* `allow-same-origin` gives it a unique opaque origin, so it cannot reach the portal's DOM/cookies/storage. The only host↔artifact channel is `window.postMessage`. The engine injects fixed runtime scripts into the served HTML at serve time (`injectArtifactScripts`, `engine/src/artifacts.ts` — the Tier-2 annotation capture, the Tier-3 layout-audit gate, (FLUX-1662) the doc-edit button runtime, and (FLUX-1667) the per-doc tab-switch runtime, as four separate `<script>` tags so one runtime's syntax error can't disable the others); the host side lives in `ArtifactPanel.tsx`. FLUX-1667: the host sends `{ns:'eh-artifact', type:'show-doc', path}` down to the iframe when the Docs panel's per-doc tab strip is clicked — the injected runtime shows exactly the `[data-eh-doc-path="<path>"]` section matching `path`, hiding the rest (defaulting to the first section present on iframe load, before any tab click).
...
| iframe → host | `doc-edit-request` | `{ path }` | **FLUX-1662.** Only ever posted by the doc-edit runtime's injected **Edit** button, which it adds to every element carrying `data-eh-doc-path` (a no-op on any artifact that doesn't emit that attribute — only the auto doc-recap does). `path` is that section's repo-relative doc path. The host opens an inline editor drawer (textarea + live `DocMarkdownPreview` + commit-message field) seeded via `GET /api/diffs/file/content`; "Commit to branch & push" calls `POST /api/diffs/file/commit` directly — no agent involvement. The drawer's edit-availability gate mirrors the Changes screen's `source.kind === 'live'` check (a live worktree must back the ticket's branch); on a race, the commit's own 409 still surfaces verbatim. |
