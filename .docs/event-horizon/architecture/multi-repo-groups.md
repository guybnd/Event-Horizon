...
| `members[].name` | string | yes | Stable short key. **Immutable once used** — it is the doc path prefix; renaming orphans `members/<name>` paths. Must be unique **and satisfy `SAFE_NAME_PATTERN`** (`/^[A-Za-z0-9][A-Za-z0-9._-]*$/`) — the loader rejects an unsafe name, invalidating the whole config. The wizard auto-derives a safe name from the folder (FLUX-543); see below. |
...
>
> **Replaced for multi-board (FLUX-1558, FLUX-1565).** Once the engine started serving multiple boards concurrently (epic FLUX-1230), the singleton became a correctness bug, not just an assumption: a board bound to workspace B could read — and fan a `group_doc` write out through — whichever group workspace A last activated. `hydrateWorkspace` (`task-store.ts`) now *also* records the result on the `Workspace` object itself (`ws.groupContext` / `ws.memberBinding`, `workspace-context.ts`). FLUX-1558 first moved the two MCP handlers that read group state — `get_project_group` and `group_doc` — onto the **bound** workspace's fields instead of the global getters; FLUX-1565 closed the remaining REST/spawn paths the same way — `routes/group.ts` and `routes/docs.ts` now resolve every group read/write off a per-request `reqWorkspace(req)` (mirroring `routes/tasks/*`'s established `req.workspace ?? getWorkspace()` pattern), `group-member-worktree.ts`'s `buildGroupDocsScopeArg` reads `getWorkspace().memberBinding`, and `task-store.ts`'s group-docs loader (`loadGroupDoc`/`loadGroupDocs`/`activeGroupStoreDir`/`startGroupDocsWatcher`) reads off the `ws: Workspace` param it already threaded through. `activateGroup`/`activateMemberBinding` still set `currentGroup`/`currentMemberBinding` too (nothing removed — some call sites, e.g. `group.ts`'s own `buildMemberScopeArgs` default and the CLI-adapter spawn helpers, still read the singleton directly), but no in-repo consumer of *group context for a specific board* depends on it anymore.
...
> - **Read in place (FLUX-405).** The member does **not** attach its own `.flux-group/` worktree (steps 1/4 above are skipped in Case 1). [`activeGroupStoreDir(ws)`](../../../engine/src/task-store.ts) falls back from `ws.groupContext` to `ws.memberBinding?.parentGroup.groupStoreDir` (FLUX-1565: the bound workspace's own fields, not the `getGroupContext()`/`getMemberBinding()` singletons), so the member loads and watches the **parent's** already-mounted store directly and renders `Product/` read-only via the existing FLUX-399 path.
