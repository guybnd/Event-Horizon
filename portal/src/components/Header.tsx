    ...
    <header className="sticky top-0 z-10 border-b border-gray-200 bg-white/65 px-4 py-3 backdrop-blur-md dark:border-white/5 dark:bg-black/20">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1 justify-end">
          ...
          {costStatsLoaded && (
            <div
              className="group flex shrink-0 items-center gap-1.5 rounded-xl border border-gray-200 bg-white/60 px-2.5 py-1.5 text-gray-500 transition-all duration-200 overflow-hidden dark:border-white/10 dark:bg-white/5 dark:text-gray-400"
              title={config?.tokenDisplayMode === 'tokens'
                ? `Lifetime tokens · ↑ ${(lifetimeTokens?.input ?? 0).toLocaleString()} / ↓ ${(lifetimeTokens?.output ?? 0).toLocaleString()}`
                : `Lifetime Claude API cost across all tickets${lifetimeTokens ? ` · ↑ ${lifetimeTokens.input.toLocaleString()} / ↓ ${lifetimeTokens.output.toLocaleString()} tokens` : ''}${lifetimeTokens?.estimated ? ' (estimated)' : ''}`}
            >
