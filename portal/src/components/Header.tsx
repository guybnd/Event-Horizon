  ...
  useEffect(() => {
    async function loadTokenStats() {
      try {
        ...
        setLifetimeCostUSD(data.lifetime?.costUSD ?? null);
      } catch {
        // non-critical
      }
    }
    ...
    <header className="sticky top-0 z-10 border-b border-gray-200 bg-white/65 px-4 py-3 backdrop-blur-md dark:border-white/5 dark:bg-black/20">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1 justify-end">
          ...
          {/* Lifetime Cost — compact stat card */}
          {lifetimeCostUSD != null && (
            <div
              ...
              title="Lifetime Claude API cost across all tickets"
            >
              <Bot className="h-3.5 w-3.5 shrink-0" />
              <span className="text-sm font-semibold leading-none">${lifetimeCostUSD.toFixed(2)}</span>
