...
export function BacklogScreen() {
  const { openTaskModal, refreshTrigger, triggerRefresh, currentProject, config, currentUser } = useApp();
    ...
    <div className="flex h-full gap-6">
      ...
      <div className="w-1/3 flex flex-col bg-white/50 dark:bg-[#1f2028]/50 rounded-2xl border border-gray-200 dark:border-white/5 overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-white/5 flex items-center justify-between bg-white/80 dark:bg-[#252630]/80">
          <h2 className="font-semibold text-gray-800 dark:text-gray-200">Backlog Items ({tasks.length})</h2>
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
          {tasks.length === 0 && (
            <p className="text-sm text-gray-500 text-center mt-10">No backlog items.</p>
          )}
          {tasks.map(task => (
            <div 
              key={task.id}
        ...
        {selectedTask ? (
          <div className="flex flex-col h-full">
            <div className="p-6 border-b border-gray-200 dark:border-white/5 flex justify-between items-start bg-white/80 dark:bg-[#252630]/80">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-1 leading-tight">{selectedTask.title || 'Untitled Task'}</h2>
                <h3 className="text-sm text-gray-400 font-bold tracking-wider">{selectedTask.id}</h3>
              </div>
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              ...
              <div>
                ...
                <div className="prose prose-sm dark:prose-invert max-w-none font-mono text-sm bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-100 dark:border-white/5 whitespace-pre-wrap">
                  {selectedTask.body || 'No description provided.'}
                </div>
              </div>
            </div>
          </div>
