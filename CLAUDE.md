...
After editing code, run **`npm run check`** from the repo root before moving a ticket to `Ready`/`Done`. It runs the type-check (portal + engine `tsc --noEmit`) and lint (`npm run lint -w portal && npm run lint -w engine`, FLUX-1073) gates and must exit 0 — neither typecheck nor lint runs as part of `dev` (the engine executes via `tsx`/esbuild, which strip types and skip lint), so a regression surfaces nowhere else. The VS Code **Problems** panel is wired to the same checks via `.vscode/tasks.json` (run the default build task, "check").

**The engine test step is TARGETED, not the full suite (FLUX-1665).** `check` ends by running `npm run test:changed -w engine` (`engine/scripts/test-changed.mjs`): the fast `unit` vitest project (no spawn, no real git fixture — pool:`threads`) always runs, plus `vitest related` for whatever `engine/src/**` files this session actually changed (committed-since-merge-base ∪ uncommitted ∪ untracked). A ticket that touched a handful of files no longer pays for the whole 205-file suite — measured ~55-96% faster than the old full-suite gate depending on how connected the touched file is, on the same Windows profile that motivated this change (see `engine/test-tiers.json` and the FLUX-1665 ticket history for the raw numbers). **Full-suite coverage is unaffected**: `npm run test -w engine` (still `vitest run`, unchanged) is what CI (`.github/workflows/ci.yml`) and release (`.github/workflows/release.yml`) call directly — neither goes through `check` — so nothing weakens at merge. Run the full suite yourself with `npm run check:full` (or `npm run test -w engine`) when you want it locally; its baseline is still being burned down — don't block on pre-existing failures unrelated to your change (e.g. `task-worktree.test.ts`'s orphaned-worktree-sweep tests are known-flaky under concurrent Windows git load, independent of what you touched).

The suite is split into two vitest projects (`engine/vitest.config.ts`) by a committed classification (`engine/test-tiers.json`), kept honest by a guard (`npm run check:classify`, wired into `check` and CI) that fails if a test file spawning a process or building a real git fixture (`mkdtemp`) is missing from the `integration` list — that guard is what stops a slow/unsafe test from silently landing in the fast tier. **Do not add `isolate: false` to the `unit` project** — it was tried and reverted: several files rely on module-level singleton/mock state that bleeds across files sharing a worker thread, and WHICH files break is non-deterministic (shifts with vitest's file-to-worker grouping). The `unit` tier's speed comes from `pool: 'threads'` (no OS process spawn) plus skipping the spawn/fixture files, not from disabling isolation.

New git-fixture-heavy tests should build their repo via the shared helper (`engine/src/test-helpers/git-fixture.ts`, `createGitFixture`/`setGitIdentity`) rather than hand-rolling `mkdtemp` + `git init` + `config` + `add` + `commit` — it clones a cached per-process template instead of repeating that sequence, and is what `task-worktree.test.ts`, `git-worktree.test.ts`, and `group-integration.test.ts` were migrated to. The long tail of the ~115 other git-fixture test files is a fast-follow, not required per-ticket.

**Windows Defender exclusion (recommended, not required).** Fixture repos live under `os.tmpdir()` with an `eh-*` prefix (`eh-git-fixture-*`, `eh-taskwt-*`, `eh-orphan-wt-*`, `eh-integ-*`, etc.). Excluding `%TEMP%\eh-*` from real-time scanning avoids Defender scanning every fixture file on creation/deletion, which is a meaningful share of the git-fixture wall-clock on Windows (the whole reason this ticket profiled the suite before making pool/tiering changes). Add the exclusion via Windows Security → Virus & threat protection → Manage settings → Add or remove exclusions → add the temp-dir path with the `eh-*` prefix (or the folder itself if your policy doesn't support prefix globs).
